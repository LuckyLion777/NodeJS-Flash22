...
// some sort of self hosted hotpixel
function logMeEverything(req, res) {
  const level = xss(req.query.level);
  const type = `everything:${xss(req.query.type)}`;

  logger(level, type, req, {});
  return res.success({});
}
