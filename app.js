...
const scrubCCfromBody = (body) => {
  if (body && body.cardNumber) {
    let toBeMasked = body.cardNumber;
    toBeMasked = `${toBeMasked.substring(0, 6)}****${toBeMasked.substring(toBeMasked.length - 4)}`;
    body.cardNumber = toBeMasked; // eslint-disable-line
  }
  return body;
};
...
const isProtectedByCloudflare = false;
...
if (isProtectedByCloudflare) { // on production, currently, on Heroku
...
app.use(expressSession({
  ...
  cookie: { // http://stackoverflow.com/a/14570856/1885921
    secure: false, // https://github.com/expressjs/session#cookiesecure
...
// commented out, because Birdman things it blocks real users - A.
// But MistaBubbles thinks it is unrelated.
...
app.use((err, req, res, next) => {
  if (err.code === 'EBADCSRFTOKEN') {
    ...
    winston.info('CSRF error : %s', err.message, {
      ...
      message: err.message,
      status: err.status,
      // stacktrace: err.stack, //because all CSRF token mismatch errors have the same stacktrace!
    });
