...
// https://www.ultratools.com/tools/ipv6CIDRToRangeResult?ipAddress=2400%3Acb00%3A%3A%2F32
  ...
  it('do works with IPv4 from cloudflare 104.20.194.45', (done) => {
    const req = new RequestMock('104.20.194.45');
    const res = new ResponseMock((error, code, message) => {
      if (error) {
        return done(error);
      }
  ...
  it('do works with IPv4 from cloudflare 104.20.195.45', (done) => {
    const req = new RequestMock('104.20.195.45');
    const res = new ResponseMock((error, code, message) => {
      if (error) {
        return done(error);
      }
